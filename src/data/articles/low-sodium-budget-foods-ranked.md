...
* **The audit was a second pull, not a vibe check.** Today's live USDA pull returned more than sodium. It also returned every fiber and protein figure those same records carry: 38 fiber values and 36 protein values across the 56 foods, 74 in total. All 74 reproduced the numbers audited into the repo CSVs last month, to the decimal, with zero mismatches. That confirms every record identity, which means the sodium value attached to each one came from the food we think it did.
