...
SCHEMA_PATH = Path(__file__).resolve().parent.parent / "migrations" / "2026-04-28-brief-tables.sql"
...
def list_missing_hero_briefs(con: sqlite3.Connection) -> list[str]:
    rows = con.execute(
        ...
        WHERE w.status IN ('written', 'reviewed') AND h.id IS NULL
        ORDER BY w.slug
        """
    ).fetchall()
...
def list_missing_pin_briefs(
    ...
    rows = con.execute(
        ...
        WHERE w.status IN ('written', 'reviewed')
        GROUP BY w.slug
        HAVING COUNT(p.id) < ?
...
def coverage_summary(con: sqlite3.Connection) -> dict:
    return {
        "total_articles": con.execute(
            "SELECT COUNT(*) FROM write_outputs WHERE status IN ('written', 'reviewed')"
        ).fetchone()[0],
        "hero_ok": con.execute(
