...
Reads topics from stage2_output (topic-research.sqlite), generates articles via
OpenRouter using the winning model from discovery (gemini-3-flash-preview by
default), runs the Layer A compliance gate, and writes accepted articles to
...
# The stage_1_5 and stage_1_75 dirs are packages; add scripts/ to sys.path so
# we can import them without requiring -m scripts.foo invocation.
if str(SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_DIR))

from stage_1_5 import openrouter as _or  # noqa: E402
import write_prompt as _prompt  # noqa: E402
import validate_article as _validate  # noqa: E402
...
def fetch_topics(
    ...
    *,
    ...
    if ranks:
        ...
        sql = (
            f"SELECT id, run_id, rank, topic, category, slug, score, rationale "
            f"FROM stage2_output WHERE run_id=? AND rank IN ({placeholders}) "
            f"ORDER BY rank"
        )
        rows = conn.execute(sql, (stage2_run_id, *ranks)).fetchall()
        return list(rows)

    limit = count if count is not None else 1
    rows = conn.execute(
        "SELECT id, run_id, rank, topic, category, slug, score, rationale "
        "FROM stage2_output WHERE run_id=? ORDER BY rank LIMIT ?",
        (stage2_run_id, limit),
    ).fetchall()
    return list(rows)
...
def insert_run(
    ...
    *,
    ...
    cur = conn.execute(
        ...
        "target_words, article_count, notes) VALUES (?, 'running', ?, ?, ?, ?, ?)",
        (now_utc_iso(), stage2_run_id, model_id, target_words, article_count, notes),
    )
    conn.commit()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate production articles from stage2_output topics.")
    p.add_argument("--run-id", type=int, default=DEFAULT_RUN_ID,
                   help=f"stage2_output.run_id to read topics from (default: {DEFAULT_RUN_ID})")
    group = p.add_mutually_exclusive_group()
    group.add_argument("--count", type=int,
                       help="Write the top-N topics by rank from the given run")
    ...
    p.add_argument("--force", action="store_true",
                   help="Overwrite existing article files (default: skip)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        log(f"write.py starting. model={args.model} "
            f"stage2_run_id={args.run_id} dry_run={args.dry_run}", fh)
        log(f"log file: {log_path}", fh)

        conn = connect_db(args.db)
        topics = fetch_topics(
            conn,
            stage2_run_id=args.run_id,
            ranks=ranks,
            count=args.count,
        )
        if not topics:
            log(f"no topics found for run_id={args.run_id} "
        ...
        for t in topics:
            ...
            if out_path.exists() and not args.force:
                log(f"SKIP existing file: {out_path.name} (use --force to overwrite)", fh)
                skipped += 1
                insert_output(conn, {
                    "run_id": run_id,
                    "topic_id": t["id"],
            ...
            for attempt in range(1, args.max_attempts + 1):
                ...
                violations = _validate.validate(markdown, slug)
            ...
            log(f"OK   #{t['rank']} {slug}  body_words={body_words}{t2_summary}{attempt_label} "
                f"lat={success['latency_ms']}ms cost=${success['cost'] or 0:.4f} "
                f"-> {out_path.relative_to(REPO_ROOT)}", fh)
            insert_output(conn, {
                "run_id": run_id, "topic_id": t["id"], "topic_rank": t["rank"],
                "topic": t["topic"], "category": t["category"], "slug": slug,
                ...
                "compliance_details": details_json, "disqualified": 0,
                "disqualify_reason": None, "file_path": str(out_path),
                "created_at": now_utc_iso(),
            })
