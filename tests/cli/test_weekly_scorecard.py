...
def test_parse_page_views_sums_only_exact_seven_day_series(page_view_payload):
    ...
    assert total == 3
    assert "event_type=browser_page_view" in source
...
def test_parse_page_views_rejects_mixed_event_series(page_view_payload):
    ...
    with pytest.raises(ValueError, match="must be browser_page_view"):
        weekly_scorecard.parse_page_views_7d(page_view_payload)

def test_startup_window_is_unavailable_not_zero(page_view_payload):
    page_view_payload["page_views_window"]["full_window_observed"] = False
    total, note = weekly_scorecard.parse_page_views_7d(page_view_payload)
    assert total is None
    assert "has not covered" in note


def test_legacy_requests_cannot_be_called_browser_views(page_view_payload):
    page_view_payload["page_views_window"]["event_type"] = "page_view"
    with pytest.raises(ValueError, match="must be browser_page_view"):
        weekly_scorecard.parse_page_views_7d(page_view_payload)
